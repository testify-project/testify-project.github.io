...
We have already seen `@Sut` annotation in action in the unit test example. In the context of a Spring integration test it serves a similar purpose, to let Testify know the field represents the system under test. In this example it happens to be the `GetGreeting` service and its interaction with `GreetingRepository` collaborator.
...
public class TestModule {
     ...
     * datasource that points to the postgres in the virtual resource.
     *
     * @param virtualResourceInstance the virtual resource instance.
     * @return the test data source
     */
    @Primary
    @Bean
    DataSource testDataSource(
            @Qualifier("postgres") VirtualResourceInstance virtualResourceInstance) {
        PGSimpleDataSource dataSource = new PGSimpleDataSource();
...
Note that the above `TestModule` simply creates a new `DataSource` based on the virtual resource our test requires via `@VirtualResource` annotation. In our `ListGreetingsResourceST` test class we requested a postgres database image and so Testify creates a `VirtualResourceInstance` whose name is derived from the virtual resource name and makes it available for injection. We can further configure the underlying client used by the `ClientInstance` by adding a method annotated with `@ConfigHandler` to the test class which takes client configuration object as a parameter. In this example we happen to be using the Jersey 2 JAX-RS client implementation and so we pass in a `ClientBuilder` instance. If you are curious about what is going on behind the scenes, it is pretty straight forward, Testify:
...
1. configures and starts required virtual resource and adds `VirtualResourceInstance` with the virtual resource's name
