...
Note that the above `TestModule` simply creates a new `DataSource` based on the container our test required via `@RequiresContainer` annotation. In our `ListGreetingsResourceST` test class we requested a postgres database image and so Testify creates a `ContainerInstance` whose name is derived from the container image name and makes it available for injection. We can further configure the underlying client used by the `ClientInstance` by adding a method annotated with `@ConfigHandler` to the test class which takes client configuration object as a paramter. In this example we happen to be using the Jersey 2 JAX-RS client implementation and so we pass in a `ClientBuilder` instance. If you are curious about what is going on behind the scenes, it is pretty straight forward, Testify:
